...
Uma API de teste para consulta de itens com paginação

##
1. Criar projeto com Spring Initializr

Para iniciarmos a criação da nossa API, vamos primeiro gerar o código do projeto Spring Boot no site: <br>
https://start.spring.io


2. Configurar banco de dados H2 no projeto

No arquivo `application.properties`, configure o banco de dados H2:
...
Arquivo import.sql para inserir itens no banco de dados H2
...
Criando a camada de Repository - ProductRepository.java
```properties
...
Criando a camada de Controller e exibindo os endpoints para acessar os itens (produtos) - ProductController.java

**OBSERVAÇÃO:**  

No exemplo abaixo, estamos retornando diretamente a **entidade** no retorno da API.  
Isso **não é uma boa prática**, pois expõe a estrutura interna do banco de dados e pode trazer problemas de manutenção ou segurança.  

O recomendado é criar um **DTO (Data Transfer Object)** para receber os objetos (como `Product`) retornados pelo método `findAll()`.  
Em seguida, a API deve retornar apenas os dados do **DTO** para o usuário, garantindo controle e segurança sobre o que é exposto.
